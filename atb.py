...
# A class that instances the classes related to one ASIC configuration and adds additional variables
# required to configure every ASIC in the system
class BoardConfig:
    # Constructor
    def __init__(self):
...
class TMP104CommunicationError(Exception):
    ...
    def __str__(self):
        return "TMP104 read error at port %d, slave %d. Chain looks open. (Data: IN = %s, OUT = %s)" % (
            self.__portID, self.__slaveID, [hex(x) for x in self.__din], [hex(x) for x in self.__dout])
...
# A class that contains all methods related to connection, control and data transmission to/from the
# system via the "daqd" interface
...
class ATB:
    ...
    # @param mode If set to 1, only data frames  with at least one event are transmitted. If set to 2,
    # all data frames are transmitted. If set to 0, the data transmission is stopped
    def start(self, ):
        nTry = 0
        while True:
            try:
    ...
    def __start(self):
        ...
        # Disable DAQ card
        self._daqdMode(0)
        # Disable gate signal from FEB/D
    ...
    # Returns a 3 element tuple with the number of transmitted, received, and error packets for a given FEB/D
    ...
    def printDataFrame(self, nonEmpty=False):
        ...
        t0 = time()
        r = False
        while (not r) and ((time() - t0) < timeout):
            wrPointer, rdPointer = self.__getDataFrameWriteReadPointer()
            # print "WR/RD pointers = %08x %08x" % (wrPointer, rdPointer)
            bs = self.__dshm.getSizeInFrames()
            while (wrPointer != rdPointer) and (not r):
                index = rdPointer % bs
                rdPointer = (rdPointer + 1) % (2 * bs)
    ...
    def sendCommand(self, portID, slaveID, commandType, payload, maxTries=10):
        ...
        reply = None
        doOnce = True
        while doOnce or (reply is None and nTries < maxTries):
            doOnce = False

            nTries = nTries + 1
            if nTries > 5:
                print "Timeout sending command. Retry %d of %d" % (nTries, maxTries)

            sn = self.__lastSN
            self.__lastSN = (sn + 1) & 0x7FFF

            rawFrame = bytearray(
                [portID & 0xFF, slaveID & 0xFF, (sn >> 8) & 0xFF, (sn >> 0) & 0xFF, commandType]) + payload
    ...
    # Defines all possible commands structure that can be sent to the ASIC and calls for
    # sendCommand to actually transmit the command
    ...
    # Defines all possible commands structure that can be sent to the ASIC and calls for sendCommand to
    ...
    def __doTOFPETAsicCommand(self, asicID, command, value=None, channel=None):
        ...
        if len(reply) < 2:
            raise tofpet.ConfigurationErrorBadReply(2, len(reply))
        status = reply[1]
    ...
    def __initializeFEBD_TOFPET(self, portID, slaveID):
        ...
        for i, asicID in enumerate(localAsicIDList):
            ...
            # So we use them as indication the ASIC is not there
    ...
    # Sends the entire configuration (needs to be assigned to the abstract ATB.config data structure) to the ASIC
    # and starts to write data to the shared memory block
    ...
    # Discards all data frames which may have been generated before the function is called. Used to
    # synchronize data reading with the effect of previous configuration commands.
    ...
    def setHVDAC_(self, channel, voltage):
        ...
        if (portID, slaveID) not in self.getActiveFEBDs():
            print "WARNING: Configuration specified for HV channel %d but FEB/D (P%02d S%02d) is not active. Skipping" \
                  % (channel, portID, slaveID)
            return
        ...
        for i, (swChannel1, (swRegion1, xi, yi, x, y, z)) in enumerate(channelMapItems):
            ...
            for swChannel2, (swRegion2, xi, yi, x, y, z) in channelMapItems[i:]:
                ...
                if hwRegion1 == hwRegion2:
                    ...
                    print "INFO: Reason: coincidences required between region %d and %d but both map to hardware trigger region %d" % (
                        swRegion1, swRegion2, hwRegion1)
                    return -1
    ...
    #        Ignored in single shot mode.
    # @param finePhase Defines the delay of the test pulse regarding the start of the frame, in units of
    #        1/392 of the clock.
    ...
    def setTestPulsePLL(self, length, interval, finePhase, invert, singleShot=False):
        if not singleShot:
            tpMode = 0b10000000
            if (interval + 1) % 32 == 0:
                raise ValueError("Test pulse interval + 1 must not be a multiple of 32")
        else:
            tpMode = 0b00000000
    ...
    # Opens the acquisition pipeline, allowing the data frames read from the shared memory block to be written
    # to disk by a writing applications
    ...
    def acquire(self, step1, step2, acquisitionTime):
        ...
        while currentFrame < stopFrame:
            ...
            if (currentFrame - lastUpdateFrame) * self.__frameLength > 0.1:
                t1 = time()
                stdout.write(
                    "Python:: Acquired %d frames in %4.1f seconds, corresponding to %4.1f seconds of data (delay = %4.1f)\r" % (
                        nFrames, t1 - t0, nFrames * self.__frameLength, (t1 - t0) - nFrames * self.__frameLength))
                stdout.flush()
                lastUpdateFrame = currentFrame
        t1 = time()
        print "Python:: Acquired %d frames in %4.1f seconds, corresponding to %4.1f seconds of data (delay = %4.1f)" % (
            nFrames, time() - t0, nFrames * self.__frameLength, (t1 - t0) - nFrames * self.__frameLength)
    ...
    # It prints warning messages if active ASICs are not being properly configured.
    def uploadConfig(self):
        for portID, slaveID in self.__getPossibleFEBDs():  # Iterate on all possible FEB/D
            if (portID, slaveID) not in self.getActiveFEBDs():
                continue

            for localAsicID, globalAsicID in enumerate(
                    self.getGlobalAsicIDsForFEBD(portID, slaveID)):  # Iterate on all possible ASIC in a FEB/D
                asicType = self.__asicType[globalAsicID]
                ac = self.config.asicConfig[globalAsicID]
                asicOK = self.__activeAsics[globalAsicID]
                if ac is None and not asicOK:
                    continue
                elif ac is None and asicOK:
                    print "WARNING: ASIC %d (P%02d S%02d A%02d) active but no config specified. Skipping" % (
                        globalAsicID, portID, slaveID, localAsicID)
                    continue
                elif ac is not None and not asicOK:
                    print "WARNING: Configuration specified for non-active ASIC %d (P%02d S%02d A%02d). Skipping" % (
                        globalAsicID, portID, slaveID, localAsicID)
                    continue

                if asicType == 0x00010001:
                    if not isinstance(ac, tofpet.AsicConfig):
                        print "WARNING: Configuration type mismatch for ASIC %d (P%02d S%02d A%02d). Skipping" % (
                            globalAsicID, portID, slaveID, localAsicID)
                        continue
